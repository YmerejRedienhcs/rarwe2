...
test('List bands', function(assert) {
  ...
  andThen(function() {
    assert.equal(
      currentURL(),
      '/bands',
      'The URL is correct after visiting /bands'
    );
    assert.equal(
      find('.band-link').length,
      2,
    ...
    assert.equal(
      ...
      1, 'The other band link contains the band name');
  });
});
...
test('create a new band', function(assert) {
  server = new Pretender(function() {
    ...
    this.post('/bands', function() {
      ...
      return [
        200,
        {
          'Content-Type': 'application/vmd.api+json'
        },
        JSON.stringify(response)
      ];
    });
    ...
    this.get('/bands/2/songs', function() {
      ...
      return [200, { 'Content-Type': 'application/vmd.api+json' }, JSON.stringify(response)];
    });
  });
  ...
  andThen (function() {
    ...
    assert.equal (
      find('.band-link:last').text().trim(),
      'Long Distance Calling',
      'Created band appears at the end of the list'
    );
    assert.equal (
      find('.nav a.active:contains("Songs")').length,
      1,
...
test('Create a new song in two steps', function(assert) {
  server = new Pretender(function() {
    this.get('/bands', function() {
      ...
      return [200, { 'Content-Type': 'applictation/vnd.api+json' }, JSON.stringify(response)];
    });
  ...
  andThen(function() {
    assert.equal(
      find('.songs .song:contains("Killer Cars")').length,
      1,
